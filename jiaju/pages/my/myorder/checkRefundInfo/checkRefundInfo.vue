...
<script>
	import { getCompanyAddrss, orderReturnApplys,orderReturn } from '@/api/myorder/request.js'
	export default {
		name: 'OrderBack',
		data(){
			return {
		...
		methods:{
			...
			orderReturnInfo(val){
				...
				.then(res=>{
					console.log(res);
					res.data.data.orderReturnApply.productAttr = JSON.parse(res.data.data.orderReturnApply.productAttr);
					res.data.data.orderReturnApply.proofPics = res.data.data.orderReturnApply.proofPics == "" ? [] :res.data.data.orderReturnApply.proofPics.split(",");
					this.info = res.data.data.orderReturnApply;
				})
			},
