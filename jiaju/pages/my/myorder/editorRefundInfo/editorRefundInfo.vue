	...
	<view class="order-back" v-if="orderInfo.orderBase">
		...
		<div class="card">
			<div class="order-title">
				<text>订单编号1: {{orderInfo.orderBase.orderSn}}</text>
			</div>
			<!-- skublock -->
		...
		<view class="btn-sub" @tap="submit">提交</view>
	</view>
</template>
	...
	export default {
		...
		methods:{
			...
			selectImg(){
				...
				uni.chooseImage({
					...
					success(res){
						...
						//暂存列表  tempFilePaths列表里的图片要放入initImgList中,
			...
			orderReturnInfo(val){
				...
				.then(res=>{
					console.log("在",res);
					res.data.data.orderItems.forEach(ele =>{
						ele.productAttr = JSON.parse(ele.productAttr)
					});
					var result = res.data.data.orderItems.find(ele =>ele.id == this.skuid);
					res.data.data.orderItems = [result];
			...
			async submit(){
				...
				.then(res =>{
					...
					if(val.length != 0 ){
						...
						SubmitChargeback(orderReturnObj).then(res=>{
							console.log(res);
							...
							if(res.data.success == true){
								...
								uni.navigateTo({
									url:"/pages/my/myorder/myorder?type=4"
								})
							}
						})
					}
